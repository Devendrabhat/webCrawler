...
def main(input_file):
    ...
    if os.path.isdir('temp'):  ###### check directory             	$current dir = ./temp
        shutil.rmtree('temp')
    ...
    for site in sites:
        ...
        # $create dir = ./temp/output{Site_no}

        os.chdir('output{}'.format(site_iterator))
        # $current dir = ./temp/output{Site_no}
        if os.path.isdir(cwd + '/siteNo{}'.format(
                site_iterator)) == True:  ###### check directory             	$current dir = ./temp
        ...
        while not q.empty():
            ...
            urls = scrapper.findAllLinks(soup, page, mainSite,
                                         UrlStatus)  ###### Finds and returns the set of urls on the page
            ...
            for url in urls:
                #		print(url)
                t.append(threading.Thread(target=linkDetails.checker, args=(
                    mainSite, url, "output{}/file{}".format(link_iterator, iterator), UrlStatus),
                                          name='t{}'.format(iterator)))
                iterator += 1
            ...
            while iterator < no:
                ...
                iterator += 1
            # @@@@@@@@@@@@@@@@@
            printingStack = filePlay.combiner(page,
                                              "output{}".format(link_iterator))  #### change slash
            #   | arg 1 !! slash above							| arg 2					!! timestamp (Day,HH,date)
            filePlay.writter(printingStack,
                             "{}/siteNo{}/{}_{}.csv".format(cwd, site_iterator, name, time.strftime("%A%d%b%y")))
            # @@@@@@@@@@@@@@@@@
            for url in urls:
